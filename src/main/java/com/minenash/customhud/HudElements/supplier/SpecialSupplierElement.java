...
public class SpecialSupplierElement implements HudElement {
    ...
    public static final Entry MAX_FPS = of(
            () -> client.options.getMaxFps().getValue() == GameOptions.MAX_FPS_LIMIT ? null
                    : client.options.getMaxFps().getValue().toString(),
    ...
    public static final Entry PROFILE_KEYBIND = of(
            () -> getActive() == null ? "" : getActive().keyBinding.getBoundKeyLocalizedText().getString(),
    ...
    public static final Entry TIME_MINUTES = of(
            () -> String.format("%02d", (int) ((ComplexData.timeOfDay % 1000) / (1000 / 60F))),
    ...
    public static final Entry TIME_SECONDS = of(
            () -> String.format("%02d", (int) ((ComplexData.timeOfDay % 1000) % (1000 / 60F) * 3.6F)),
    ...
    public static final Entry OFFHAND_ITEM = of(
            () -> I18n.translate(client.player.getOffHandStack().getItem().getTranslationKey()),
    ...
    public static final Entry ACTIVE_RENDERER = of(() -> {
        var r = renderer();
        return r != null ? r.getClass().getSimpleName() : "none (vanilla)";
    },
            () -> {
                var r = renderer();
                return r != null ? r.getClass().getSimpleName().length() : 7;
            },
            () -> renderer() != null);
    ...
    public record Entry(Supplier<String> stringSupplier, Supplier<Number> numberSupplier,
            Supplier<Boolean> booleanSupplier) {
    }

    public static Entry of(Supplier<String> stringSupplier, Supplier<Number> numberSupplier,
            Supplier<Boolean> booleanSupplier) {
